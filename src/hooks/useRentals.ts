...
export function useCreateRental() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      productItems: { product_id: string; quantity: number }[];
    }) => {
      // Create rental (rental_code is auto-generated by trigger)
      ...
      // Add products with quantities to rental
      if (productItems.length > 0) {
        const rentalProducts = productItems.map((item) => ({
